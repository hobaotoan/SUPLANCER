<template>
    <div class="view-assess-project animate__animated animate__fadeIn my-4">
        <div class="assess-project__title me-5 col-12">
            <h2
        ...
        <div class="assess-project__detail my-4">
            <p class="assess-project__date" v-if="startDate">
            ...
            <p class="assess-project__status" v-if="startDate">
                ...
                <span style="color: green" v-if="completedAssess">
                    Đã hoàn thành đánh giá</span
                >
                <span style="color: red" v-else> Chưa hoàn thành đánh giá</span>
            </p>
        </div>
        <div
            class="table-assess border rounded-4 p-3 col-8 d-flex flex-column m-auto"
            v-if="startDate"
        >   
            <table class="table">
                <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row">I.1</th>
                        <td>
                            Thực hiện nội quy của cơ quan (nếu thực tập online
                            thì không chấm điểm)
                        </td>
                        <td>{{ points[1] }}</td>
                    </tr>
                    <tr>
                        <th scope="row">I.2</th>
                        <td>
                            Chấp hành giờ giấc làm việc (nếu thực tập online thì
                            không chấm điểm)
                        </td>
                        <td>{{ points[2] }}</td>
                    </tr>
                    <tr>
                        <th scope="row">I.3</th>
                        <td>
                            Thái độ giao tiếp với cán bộ trong đơn vị (nếu thực
                            tập online thì không chấm điểm)
                        </td>
                        <td>{{ points[3] }}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            Báo cáo tiến độ công việc cho cán bộ hướng dẫn mỗi
                            tuần 1 lần
                        </td>
                        <td>{{ points[10] }}</td>
                    </tr>
            ...
            <div class="comment">
                <p><strong>Nhận xét: </strong>{{ projectAssess.message }}</p>
            </div>
        </div>
    </div>
</template>
