<template>
    <div class="assess-leader animate__animated animate__fadeIn my-4">
        <div class="assess-leader__title me-5 col-12">
            <h2
        ...
        <div class="assess-leader__detail my-4">
            <p class="assess-leader__date">
                <strong>Thời gian làm việc:</strong>
                <span> từ ngày {{ startDate }}</span>
            </p>
            <p class="assess-leader__status" v-if="completedAssess">
...
export default {
    ...
    async created() {
        ...
        await this.$store.dispatch("setLeaderAssess", payload);

        // Deep Clone diem danh gia cua giang vien!
        this.assessPoint = JSON.parse(JSON.stringify(this.leaderAssess));
    ...
    methods: {
        ...
        async assess(event) {
            ...
            const response = await axios.put(
                `${config.domain}/leader/assess-result`,
